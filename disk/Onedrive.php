...
class Onedrive {
    ...
    public function list_files($path = '/')
    {
        ...
        if (!($files = getcache('path_' . $path, $this->disktag))) {
            ...
            $url .= '?top=1000&expand=children(select=id,name,size,file,folder,parentReference,lastModifiedDateTime,'.$this->DownurlStrName.')';
                ...
                if (isset($files['file'])) {
                    if (in_array(strtolower(splitlast($files['name'],'.')[1]), $exts['txt'])) {
                        if ($files['size']<1024*1024) {
                            if (!(isset($files['content'])&&$files['content']['stat']==1000)) {
